...
export class PatternMatchingAdapter implements PatternMatchingPort, PrimaryAdapter {
  private storageAdapter: PatternStoragePort;
  private patternCache: Map<string, AutomationPatternData[]> = new Map();
  ...
  public async handleAutomationRequest(request: AutomationRequest): Promise<AutomationPatternMatched | null> {
    ...
    if (bestMatch && this.isMatchAcceptable(bestMatch)) {
      return new AutomationPatternMatched(
        bestMatch.pattern,
        request,
  ...
  public async findMatchingPatterns(request: AutomationRequest): Promise<PatternMatch[]> {
  ...
  public selectBestPattern(matches: PatternMatch[]): PatternMatch | null {
    ...
    const lowConfidenceMatches = matches.filter(m => 
      m.recommendationLevel === 'low' && m.overallScore > 0.8
    );
  ...
  public async executePattern(pattern: AutomationPatternData, request: AutomationRequest): Promise<ExecutionResult> {
  ...
  public async updatePatternStatistics(patternId: string, result: ExecutionResult): Promise<void> {
    try {
  ...
  public async analyzePatternPerformance(patternId: string): Promise<{
    pattern: AutomationPatternData;
    performance: {
      successRate: number;
  ...
  public async getPatternRecommendations(context: ExecutionContext): Promise<{
    ...
    const suggestionsByType: Record<string, PatternMatch[]> = {};
  ...
  private async evaluatePatternMatch(pattern: AutomationPatternData, request: AutomationRequest): Promise<PatternMatch> {
  ...
  private createLowScoreMatch(pattern: AutomationPatternData, reason: string): PatternMatch {
    return {
      pattern,
      confidence: 0,
      contextScore: 0,
  ...
  private calculatePayloadSimilarity(patternPayload: Record<string, any>, requestPayload: Record<string, any>): number {
    const patternKeys = Object.keys(patternPayload);
    const requestKeys = Object.keys(requestPayload);
    
    if (patternKeys.length === 0 && requestKeys.length === 0) return 1.0;
  ...
  private calculateContextScore(patternContext: ExecutionContext, requestContext: ExecutionContext): number {
    let score = 0;
    let totalWeight = 0;
  ...
  private calculatePatternReliability(pattern: AutomationPatternData): number {
  ...
  private calculateAgePenalty(pattern: AutomationPatternData): number {
    const ageInDays = (Date.now() - pattern.context.timestamp.getTime()) / (1000 * 60 * 60 * 24);
    
    if (ageInDays <= 1) return 1.0;      // Fresh patterns
    if (ageInDays <= 7) return 0.95;     // Week-old patterns
    if (ageInDays <= 30) return 0.85;    // Month-old patterns
    if (ageInDays <= 90) return 0.7;     // Quarter-old patterns
    
    return 0.5; // Very old patterns get significant penalty
  }

  private determineRecommendationLevel(overallScore: number, pattern: AutomationPatternData): 'high' | 'medium' | 'low' | 'risky' {
    const successRate = pattern.usageCount > 0 ? pattern.successfulExecutions / pattern.usageCount : 1;
    
    if (overallScore >= 0.8 && successRate >= 0.8) return 'high';
    if (overallScore >= 0.6 && successRate >= 0.6) return 'medium';
    if (overallScore >= 0.4 && successRate >= 0.4) return 'low';
  ...
  private async getPatternsByType(messageType: string): Promise<AutomationPatternData[]> {
  ...
  private invalidateCacheForType(messageType: string): void {
    this.patternCache.delete(messageType);
  }

  private isMatchAcceptable(match: PatternMatch): boolean {
    return match.recommendationLevel !== 'risky' && match.overallScore >= 0.5;
  }

  private isContextValid(patternContext: ExecutionContext, currentContext: ExecutionContext): boolean {
  ...
  private isElementActionable(element: Element, actionType: string): boolean {
  ...
  private async performPatternAction(element: Element, pattern: AutomationPatternData, request: AutomationRequest): Promise<void> {
    switch (pattern.messageType) {
      case 'FillTextRequested':
        await this.fillTextAction(element as HTMLInputElement, request.payload.value || '');
        break;
      
      case 'ClickElementRequested':
      case 'SelectProjectRequested':
      case 'SelectChatRequested':
        await this.clickAction(element as HTMLElement);
        break;
  ...
  private async fillTextAction(input: HTMLInputElement, value: string): Promise<void> {
    input.focus();
    input.value = '';
  ...
  private async clickAction(element: HTMLElement): Promise<void> {
  ...
  private calculateUpdatedConfidence(pattern: AutomationPatternData, result: ExecutionResult): number {
  ...
  private calculateReliabilityLevel(pattern: AutomationPatternData): 'high' | 'medium' | 'low' | 'unreliable' {
    const successRate = pattern.usageCount > 0 ? pattern.successfulExecutions / pattern.usageCount : 1;
    const ageInDays = (Date.now() - pattern.context.timestamp.getTime()) / (1000 * 60 * 60 * 24);
    ...
    // Age penalty
    if (ageInDays > 30) score *= 0.7;
    else if (ageInDays > 7) score *= 0.9;
  ...
  private getRecommendedAction(pattern: AutomationPatternData): 'keep' | 'retrain' | 'delete' {
    const reliability = this.calculateReliabilityLevel(pattern);
    const successRate = pattern.usageCount > 0 ? pattern.successfulExecutions / pattern.usageCount : 1;
  ...
  private shouldRetrain(pattern: AutomationPatternData): boolean {
    const successRate = pattern.usageCount > 0 ? pattern.successfulExecutions / pattern.usageCount : 1;
    const ageInDays = (Date.now() - pattern.context.timestamp.getTime()) / (1000 * 60 * 60 * 24);
    
    return (
      (pattern.usageCount >= 3 && successRate < 0.5) ||
      (ageInDays > 14 && pattern.usageCount === 0) ||
      pattern.confidence < 0.4
    );
  }

  private isPatternStale(pattern: AutomationPatternData): boolean {
    const ageInDays = (Date.now() - pattern.context.timestamp.getTime()) / (1000 * 60 * 60 * 24);
    return ageInDays > 30;
  }
