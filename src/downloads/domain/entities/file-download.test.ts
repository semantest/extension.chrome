...
describe('FileDownload', () => {
  ...
  describe('validation', () => {
    ...
    it('should throw error when updating progress on non-downloading status', () => {
      expect(() => download.updateProgress(50)).toThrow(
        'Cannot update progress - download not in progress'
      );
    });
    ...
    it('should validate progress range', () => {
      ...
      expect(() => download.updateProgress(-1)).toThrow(
        'Progress must be between 0 and 100'
      );
      
      expect(() => download.updateProgress(101)).toThrow(
        'Progress must be between 0 and 100'
      );
    });
    ...
    it('should not allow operations on completed download', () => {
      ...
      expect(() => download.pause()).toThrow(
        'Cannot pause - download not in progress'
      );
    });
  });
