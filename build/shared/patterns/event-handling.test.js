 ...
 */
import { TypedEventEmitter, EventBus, MessageRouter } from './event-handling';
describe('Event Handling Patterns', () => {
    ...
    describe('MessageRouter', () => {
        ...
        it('should route messages to handlers', async () => {
            ...
            const result = await router.route({ action: 'test', data: 'hello' }, {});
            expect(handler.handle).toHaveBeenCalledWith({ action: 'test', data: 'hello' }, {});
            expect(result).toEqual({ success: true });
        });
        it('should throw error for unregistered action', async () => {
            await expect(router.route({ action: 'unknown' }, {})).rejects.toThrow('No handler registered for action: unknown');
        });
        it('should handle synchronous handlers', async () => {
            ...
            const result = await router.route({ action: 'sync' }, {});
        ...
        it('should setup chrome message listener', () => {
            ...
            router.register('test', handler);
            const result = listener({ action: 'test' }, {}, sendResponse);
