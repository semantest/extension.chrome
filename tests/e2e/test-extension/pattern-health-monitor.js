...
export class PatternHealthMonitor {
    ...
    async performHealthCheck(patternId) {
        ...
        // Track health check
        globalMessageStore.addInboundMessage('PATTERN_HEALTH_CHECK_COMPLETED', {
            patternId,
            reportId: report.id,
    ...
    async executeTestSuite(testSuiteId) {
        ...
        globalMessageStore.addInboundMessage('PATTERN_TEST_SUITE_EXECUTED', {
            testSuiteId,
    ...
    async runValidationRule(rule, pattern) {
        try {
            const result = await globalPerformanceOptimizer.timeExecution(() => rule.validator(pattern), 'messageProcessingTime');
            return {
                id: `${rule.id}-${Date.now()}`,
                name: rule.name,
