...
export class TimeTravelUI {
    ...
    subscribeToStore() {
        this.unsubscribe = globalMessageStore.subscribe((state) => {
            this.updateUI(state);
        });
    }
    updateUI(state) {
        ...
        if (backBtn)
            backBtn.disabled = !globalMessageStore.canTimeTravelBack();
        if (forwardBtn)
            forwardBtn.disabled = !globalMessageStore.canTimeTravelForward();
        if (resetBtn)
...
// Console helper
window.showTimeTravelUI = () => globalTimeTravelUI.show();
